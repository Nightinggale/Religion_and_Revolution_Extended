...
void CvGameTextMgr::setCityBarHelp(CvWStringBuffer &szString, CvCity* pCity)
{
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		if (aiYields[iYield] > 0 && (YieldTypes) iYield != YIELD_CULTURE) 
		{
			if (bFirst)
			{
				bFirst = false;
...
void CvGameTextMgr::setBasicUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText)
{
	...
	//TAC Whaling, ray
	if (unitClassIntToBeChecked == GC.getDefineINT("UNITCLASS_WHALING_BOAT"))
	{
		szBuffer.append(NEWLINE);
	...
	//End TAC Whaling, ray
	// R&R, ray, High Sea Fishing - START
	if (unitClassIntToBeChecked == GC.getDefineINT("UNITCLASS_FISHING_BOAT"))
	{
		szBuffer.append(NEWLINE);
