...
//VET NewCapacity - end 2/7
					...
					if (iModifiedOutput > iHighestOutput)
					{
						if (iOutput != 0)
						{
							...
							if (iOutput > 0)
							{
								...
								if (eYieldProduced == YIELD_HORSES || eYieldProduced == YIELD_MUSKETS || eYieldProduced == YIELD_CANNONS || eYieldProduced == YIELD_TOOLS || eYieldProduced == YIELD_FOOD)
								{
									bIsMilitary = true;
								}
								iValue += iTempValue;
	...
	// TAC - AI Buildings - koma13 - END
	{
		...
		for (int i = 0; i < NUM_YIELD_TYPES; ++i)
		{
			...
			if (iAdded != 0)
			{
				if (eLoopYield == YIELD_HORSES || eLoopYield == YIELD_MUSKETS || eLoopYield == YIELD_CANNONS || eLoopYield == YIELD_TOOLS || eLoopYield == YIELD_FOOD)
				{
					bIsMilitary = true;
				}
