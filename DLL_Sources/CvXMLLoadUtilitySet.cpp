...
bool CvXMLLoadUtility::SetPostGlobalsGlobalDefines()
{
	...
	if (GC.getDefinesVarSystem()->GetSize() > 0)
	{
		...
		SetGlobalDefine("BUILDING_FORT", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("BUILDING_FORT", idx);

		SetGlobalDefine("BUILDING_FORTRESS", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("BUILDING_FORTRESS", idx);

		SetGlobalDefine("BUILDING_STRONGHOLD", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("BUILDING_STRONGHOLD", idx);

		SetGlobalDefine("BUILDING_CITADEL", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("BUILDING_CITADEL", idx);

		SetGlobalDefine("BUILDING_GREAT_BASTION", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("BUILDING_GREAT_BASTION", idx);

		SetGlobalDefine("UNITCLASS_LIGHT_CANNON", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("UNITCLASS_LIGHT_CANNON", idx);

		SetGlobalDefine("UNITCLASS_CANNON", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("UNITCLASS_CANNON", idx);

		SetGlobalDefine("UNITCLASS_HESSIAN", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("UNITCLASS_HESSIAN", idx);

		SetGlobalDefine("PROFESSION_SOLDIER", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("PROFESSION_SOLDIER", idx);

		SetGlobalDefine("PROFESSION_MUSKETMAN", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("PROFESSION_MUSKETMAN", idx);
		...
		// R&R, ray, Changes Entering Peaks - START
		SetGlobalDefine("UNITCLASS_SCOUT", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("UNITCLASS_SCOUT", idx);

		SetGlobalDefine("PROFESSION_PIONEER", szVal);
		idx = FindInInfoClass(szVal);
		GC.getDefinesVarSystem()->SetValue("PROFESSION_PIONEER", idx);
		// R&R, ray, Changes Entering Peaks - END
