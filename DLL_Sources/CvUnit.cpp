...
void CvUnit::init(int iID, UnitTypes eUnit, ProfessionTypes eProfession, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, int iYieldStored)
{
	...
	//Neuer Code
	//identifiziere General anhand LeaderExperience
	if(GC.getUnitInfo(getUnitType()).getLeaderExperience() == 20) {
...
bool CvUnit::canMoveInto(const CvPlot* pPlot, bool bAttack, bool bDeclareWar, bool bIgnoreLoad) const
{
	...
	case DOMAIN_LAND:
		...
		{
			if (!isNative() && (!m_pUnitInfo->isAnimal()) && (m_pUnitInfo->getUnitClassType() != GC.getDefineINT("UNITCLASS_RANGER")) && (m_pUnitInfo->getUnitClassType() != GC.getDefineINT("UNITCLASS_NATIVE_MERC")) && (m_pUnitInfo->getUnitClassType() != GC.getDefineINT("UNITCLASS_SCOUT")) && (getProfession() != (ProfessionTypes)GC.getDefineINT("PROFESSION_PIONEER"))) 
			{
				return false;
			}
		}
...
bool CvUnit::doFound(bool bBuyLand)
{
	...
	if (!isHuman() && eParent != NO_PLAYER)
	{
		...
		UnitTypes eCheapestDefender = NO_UNIT;
		...
		for (int iUnitClass = 0; iUnitClass < GC.getNumUnitClassInfos(); ++iUnitClass)
		{
			...
			if (eLoopUnit != NO_UNIT)
			{
				if (GC.getUnitInfo(eLoopUnit).getDefaultUnitAIType() == UNITAI_DEFENSIVE)
				{
					...
					{
						iLowestCost = iCost;
						eCheapestDefender = eLoopUnit;
					}
				}
			}
		}

		if (eCheapestDefender != NO_UNIT)
		{
			for (int iI = 0; iI < GC.getAI_FREE_CITY_DEFENDERS(); iI++)
			{
				CvUnit* pUnit = kPlayer.initEuropeUnit(eCheapestDefender);
				FAssert(pUnit != NULL);
...
bool CvUnit::canBuild(const CvPlot* pPlot, BuildTypes eBuild, bool bTestVisible) const
{
	...
	if (!isHuman())
	{
		if (pPlot->getFeatureType() != NO_FEATURE && pPlot->getBonusType() != NO_BONUS)
		{		
...
{
	return (GC.getUNIT_VISIBILITY_RANGE() + getExtraVisibilityRange());
}
...
bool CvUnit::canHaveProfession(ProfessionTypes eProfession, bool bBumpOther, const CvPlot* pPlot) const
{
	...
	// R&R, ray, lock Missionary for Criminals, African Slaves and Native Slaves - START
	if (kOwner.isHuman() && GC.getUSE_NEW_RULES_FOR_PROFESSION_MISSIONARY() == 1 && GC.getProfessionInfo(eProfession).getMissionaryRate() > 0)
	{
		int unitClassIntToBeChecked = m_pUnitInfo->getUnitClassType();
		if (unitClassIntToBeChecked == GC.getDefineINT("UNITCLASS_PRISONER") || unitClassIntToBeChecked == GC.getDefineINT("UNITCLASS_NATIVE_SLAVE") || unitClassIntToBeChecked == GC.getDefineINT("UNITCLASS_AFRICAN_SLAVE"))
		{
		...
		if (pCity != NULL)
		{
			if (pCity->getYieldBuilding(YIELD_CROSSES) == NO_BUILDING)
			{
				return false;
			}
	...
	// R&R, ray, lock Missionary for Criminals, African Slaves and Native Slaves - END
	...
	if (pCity != NULL)
	{
		 if (!pCity->AI_isWorkforceHack())
		 {
			...
			{
				// R&R, ray, Abandon City, START
				if (pCity->getPopulation() <= 1 && !isHuman()) // allow only humans to disband cities
				{
					return false;
				}

				if (pCity->getPopulation() <= 1 && isHuman())
				{
					bool hasWar = false;
...
{
	return (getUnitClassType() == (UnitClassTypes)GC.getDefineINT("UNITCLASS_FISHING_BOAT"));
}
// R&R, ray, High Sea Fishing - END
